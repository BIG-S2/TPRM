%This code runs Scenario 1 Section 3.1 described in the paper:
...
%Command makeBall from k-wave-toolbox
ball_10 = makeBall(dimx, dimy, dimz, 25, 25, 25, 4);
ball_11 = makeBall(dimx, dimy, dimz, 25, 25, 25, 3);
ball_12 = makeBall(dimx, dimy, dimz, 25, 25, 25, 2);
ball_13 = makeBall(dimx, dimy, dimz, 25, 25, 25, 1);

ball_20 = makeBall(dimx, dimy, dimz, 32, 32, 32, 4);
ball_21 = makeBall(dimx, dimy, dimz, 32, 32, 32, 3);
ball_22 = makeBall(dimx, dimy, dimz, 32, 32, 32, 2);
ball_23 = makeBall(dimx, dimy, dimz, 32, 32, 32, 1);


ball_1=ball_10+ball_11+ball_12+ball_13;
ball_2=ball_20+ball_21+ball_22+ball_23;

ball=ball_1+ball_2; 

auxball=zeros(size(ball));
...
N=200;
TImg2=zeros(64,64,50,N);%Store tensor data
Y=binornd(1,0.5,N,1);
TI=zeros(64,64,50,N); 
...
for i=1:N
    if Y(i,1)==0
        TI(:,:,:,i)=double(A)+20*auxball+cc*randn(64,64,50);
    else
        TI(:,:,:,i)=double(A)+cc*randn(64,64,50);
    end
end
for i=1:N
...
% 3) TPRM
 %Rank inside the partitions, it can be RR=R;
indexTra=training(CVP,sim);
indexTest=test(CVP,sim);
...
PredictedY=(normcdf(Cov_test*mean(betamat(:,2001:50:end),2)))>=0.5;
...
save(strcat('Results_Scenario1_R',num2str(R),'_RR',num2str(RR),'.mat'))
