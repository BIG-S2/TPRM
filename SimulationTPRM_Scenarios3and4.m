%This code runs Scenario 3 and 4 (with modifications indicated on the comments) 
%    Section 3.1 described in the paper:
...
for i=1:N
    if Y(i,1)==0
        TI(:,:,:,i)=double(A)+SignalConst*auxball+cc*randn(64,64,50);
    else
...
%We fixed R but it can be computed  to retain 99% of the cariance or any
...
save(strcat('Results_Scenario4_R',num2str(R),'_RR',num2str(RR),'.mat'))
